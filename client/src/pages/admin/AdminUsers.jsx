...
export default function AdminUsers() {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8 text-slate-900 dark:text-[#F8FAFC]">
      ...
      {actionMsg && (
        <div className="p-3.5 bg-emerald-500/10 border border-emerald-500/30 text-emerald-700 dark:text-emerald-400 text-xs font-mono font-bold rounded-xl flex items-center">
          <UserCheck className="w-4 h-4 mr-2" /> {actionMsg}
        </div>
      )}

      {/* Search & Filter Bar */}
      <div className="tech-card p-4 flex flex-col md:flex-row items-center justify-between gap-3 bg-white dark:bg-[#0F172A] border-slate-200 dark:border-[#1E293B]">
        ...
        <div className="text-xs font-mono text-slate-500 dark:text-[#94A3B8] flex items-center">
          <ShieldCheck className="w-4 h-4 mr-1.5 text-emerald-600 dark:text-[#10B981]" />
      ...
      <div className="tech-card overflow-hidden bg-white dark:bg-[#0F172A] border-slate-200 dark:border-[#1E293B]">
        <div className="p-4 bg-slate-50 dark:bg-[#0F172A] border-b border-slate-200 dark:border-[#1E293B] font-mono font-bold text-xs text-slate-900 dark:text-[#F8FAFC] flex justify-between items-center">
          <span className="flex items-center">
            <Users className="w-4 h-4 mr-2 text-blue-600 dark:text-[#38BDF8]" /> Registered Student Accounts ({users.length})
          </span>
        </div>

        {loading ? (
          <div className="p-8 text-center text-xs font-mono text-slate-500 dark:text-[#94A3B8]">Loading student accounts...</div>
        ) : users.length === 0 ? (
          <div className="p-8 text-center text-xs font-mono text-slate-500 dark:text-[#94A3B8]">No student accounts found matching search criteria.</div>
        ) : (
