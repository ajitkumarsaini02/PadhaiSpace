...
export default function AdminSubjects() {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8 text-[#F8FAFC]">
      ...
      <div className="tech-card p-6 space-y-4">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              <label className="block text-xs font-mono font-bold text-[#F8FAFC] mb-1">Subject Name *</label>
              <input
                type="text"
                value={form.name}
            ...
            <div>
              <label className="block text-xs font-mono font-bold text-[#F8FAFC] mb-1">Subject Code *</label>
              <input
                type="text"
                value={form.code}
                ...
                required
                className="w-full px-3.5 py-2.5 text-xs bg-[#070A12] text-[#F8FAFC] border border-[#1E293B] rounded-xl focus:outline-none focus:border-[#38BDF8] font-semibold uppercase"
              />
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div>
              <label className="block text-xs font-mono font-bold text-[#F8FAFC] mb-1">Thumbnail URL</label>
              <input
                type="text"
      ...
      <div className="tech-card overflow-hidden space-y-4 p-4">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-2 border-b border-[#1E293B]">
          <div className="font-mono font-bold text-xs flex items-center">
            <BookOpen className="w-4 h-4 mr-2 text-[#38BDF8]" /> All Subjects ({displayedSubjects.length})
          </div>
          ...
          <div className="relative w-full sm:w-64">
            ...
            <input
              ...
              className="w-full pl-9 pr-3 py-1.5 text-xs bg-[#070A12] text-[#F8FAFC] border border-[#1E293B] rounded-xl focus:outline-none focus:border-[#38BDF8] font-mono"
            />
          </div>
        </div>

        {displayedSubjects.length === 0 ? (
          <div className="py-8 text-center text-xs text-[#94A3B8] font-mono">No subjects found.</div>
        ) : (
          <div className="divide-y divide-[#1E293B]">
            {displayedSubjects.map((s) => (
              <div key={s._id} className="py-3 px-2 flex items-center justify-between hover:bg-[#0F172A] text-xs transition-colors rounded-xl">
                <div>
                  <div className="flex flex-wrap items-center gap-2">
                    <span className="font-bold text-[#F8FAFC] text-sm">{s.name}</span>
                    {s.code && (
                  ...
                  {s.description && <p className="text-[#94A3B8] mt-1">{s.description}</p>}
                </div>

                <div className="flex items-center space-x-2">
                  <Link
                    to={`/subjects/${s._id}`}
                    className="p-2 text-[#38BDF8] hover:bg-[#1E293B] rounded-lg transition-colors font-mono font-bold text-[11px]"
