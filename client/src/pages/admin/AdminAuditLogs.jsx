  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8 text-[#F8FAFC]">
      ...
      <div className="tech-card overflow-hidden">
        <div className="p-4 bg-[#0F172A] border-b border-[#1E293B] font-mono font-bold text-xs flex justify-between items-center text-[#F8FAFC]">
          <span className="flex items-center"><History className="w-4 h-4 mr-2 text-[#38BDF8]" /> Admin Audit Entries ({logs.length})</span>
          <button
            onClick={fetchAuditLogs}
            className="p-1.5 bg-[#0B0F19] hover:bg-[#1E293B] text-[#38BDF8] border border-[#1E293B] rounded-lg transition-colors cursor-pointer text-xs font-mono flex items-center"
          >
        ...
        {loading ? (
          <div className="p-8 text-center text-xs font-mono text-[#94A3B8]">Loading security logs...</div>
        ) : (
            ...
            <table className="w-full text-left border-collapse text-xs">
              ...
              <tbody className="divide-y divide-[#1E293B] font-medium">
                {logs.length === 0 ? (
                  ...
                  logs.map((log) => (
                    <tr key={log._id} className="hover:bg-[#0F172A] transition-colors">
